...
do
    ...
    local kyrianAbilities = {307865, 304971, 308491, 323547, 325013, 312202, 324386, 307443, 321321, 310454, 326434,
                             306830};
    local venthyrAbilities = {317349, 316958, 324149, 323654, 323673, 311648, 320674, 314793, 321792, 326860, 323546,
                              317009};
    local nightFaeAbilities = {325886, 328620, 328231, 328305, 327661, 324128, 328923, 314791, 325640, 327104, 323764,
                               323639};
    local necrolordAbilities = {324143, 328204, 325028, 328547, 324724, 315443, 326059, 324220, 325289, 325216, 325727,
                                329554};

    -- 盟约标志技能列表id[格里恩,温西尔,法夜,通灵]
            ...
            if ability.zoneAbilityID == constructAbilityZoneID then
                ...
                if zoneAbilityFrameHooked == false then
                    ...
                    ZoneAbilityFrame:HookScript("OnUpdate", function(self, ...)
                        if constructAbilityFound == true and signatureAbilityOnActionbar == true and
                            otherActiveZoneAbilityFound == false then
                            self:Hide();
                        end
                    end)
    ...
    function CAC_checkSignatureAbilityOnActionbar(slot)
        ...
        if signatureAbilityRemovedFromBar == true and
            (constructAbilityFound == true or otherActiveZoneAbilityFound == true) then
            ZoneAbilityFrame:Show()
        end
    end

    ---[Documentation]()获取每个盟约的技能和标志技能
    ---@return string classAbilityNameTooltip 盟约技能图标
    ---@return string classAbilityMacroStr 盟约技能宏代码
    ---@return string signatureAbilityNameTooltip 盟约标志技能图标
    ---@return string signatureAbilityMacroStr 盟约标志技能宏代码
    function CAC_getSpellNames()
        if playerCovenant == 1 then
            ...
            -- 将格里恩盟约更改为指向宏
            classAbilityMacroStr = "[@player,mod:alt] " .. classAbilityName .. " [@cursor] " .. classAbilityName;
            ...
            if signatureAbilityName == nil then
                ...
                signatureAbilityNameTooltip = "[nomod:alt] " .. signatureAbilityName .. "; [mod:alt]item:" ..
                                                  phialItemID;
                --- 不按 alt 召唤执事者马仔，按住alt+宏 嗑瓶子.
        ...
        elseif playerCovenant == 2 then
            ...
            if signatureAbilityName == nil then
                ...
                signatureAbilityNameTooltip = "[nomod:ctrl] " .. signatureAbilityName .. "; [mod:ctrl]item:" ..
                                                  dukesTeaItemID;
                signatureAbilityMacroStr =
                    "[@cursor,mod:alt,nomod:ctrl] " .. signatureAbilityName .. "; [nomod:ctrl] " .. signatureAbilityName ..
        ...
        elseif playerCovenant == 4 then
            ...
            signatureAbilityNameTooltip = "[nomod:alt] " .. signatureAbilityName .. "; [mod:alt]" ..
                                              constructAbilityName;
        ...
        if event == "ACTIONBAR_SLOT_CHANGED" then
            ...
            if event == "PLAYER_LOGIN" or event == "COVENANT_CHOSEN" then
                ...
                if InCombatLockdown() == true then
                    print(
                        string.format("%sCannot update covenant macros due to combat lockdown...", CACADDON_CHAT_TITLE));
                else
                    ...
                    if foundClassMacro == true then
                        ...
                        print(string.format("%sExisitng macro for class ability not found. Creating new one...",
                            CACADDON_CHAT_TITLE));
                        ...
                        if GetNumMacros() < MAX_ACCOUNT_MACROS then
                            ...
                            print(string.format("%sCould not create macro for class ability. Macro limit reached.",
                                CACADDON_CHAT_TITLE));
                        end
                    end
                    ...
                    if foundSignatureMacro == true then
                        ...
                        print(string.format("%sExisitng macro for signature ability not found. Creating new one...",
                            CACADDON_CHAT_TITLE));
                        -- Make sure the user has not reached cap on number of macros
                        if GetNumMacros() < MAX_ACCOUNT_MACROS then
                            -- Create the macro
                            ...
                            print(string.format("%sCould not create macro for signature ability. Macro limit reached.",
                                CACADDON_CHAT_TITLE));
                        end
                    end
                end
